...
- `fathom execute` (live context): **first (WS0-T06) loads the latest `preflight_attestations` row** and refuses unless it exists and is `attested=True`, `go=True`, `pre_cutover=False`, for the configured `oanda_account_id`, and `created_at` within **24h** of now — the persisted link between the operator's `fathom preflight --attest-track-record` ceremony and this order. The refusal happens before the in-process preflight, the typed confirm, sizing and submission (fail closed, exit 1, no order) and names the remedy. It then runs `run_preflight` for the mechanical checks with `attested=<the persisted value>` (never a hardcoded `True`), then requires a typed confirmation — the operator types the `oanda_account_id` (a plain `str`, safe to echo — not the SecretStr token). This live confirm is a **distinct prompt evaluated before `assert_live_allowed`, NOT guarded by `--yes`/`skip_confirm`** (N-3): the existing `--yes`-gated `[y/N]` confirm (`cli.py:1625`) remains **demo-only**; live always requires the typed account id even with `--yes`. Then calls `assert_live_allowed(...)` before sizing; sizes with `effective_risk_fraction(settings)`. Any gate failing ⇒ refuse with the named reason + non-zero exit, **no order placed**. On demo, `execute` behaves exactly as Phase 3 (no preflight, no typed confirm, 0.25% fraction).
...
- [ ] **(WS0-T06)** A live `fathom execute` refuses — exit non-zero, `submit_order` and `size_position` never called, `run_preflight` never reached — when the latest persisted attestation is missing, older than 24h, `attested=False`, `go=False`, `pre_cutover=True`, or recorded for a different account. A fresh non-pre-cutover attested GO for the configured account clears the check and `run_preflight` receives `attested=True` from that row. A `--pre-cutover` GO therefore satisfies runbook Step 2 but never authorizes execution.
...
sequenceDiagram
    ...
    CLI->>CLI: load latest preflight_attestations row
    Note over CLI: missing / >24h / not attested / NO-GO /<br/>pre-cutover / wrong account => REFUSE, no order
    CLI->>PF: run_preflight(..., attested=<persisted>)
