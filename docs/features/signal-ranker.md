...
6. **Rank (D-P2-4 / AMBIGUOUS-04 ruling):** sort survivors by **`oos_sharpe_mean` descending (primary)**, then **`quality_score` descending (tie-break)**. `quality_score` is *not* multiplied into a cross-strategy composite — `oos_sharpe_mean` is the INV-11-comparable validated number; `quality_score` (per-strategy [0,1]) only orders ties. Assign 1-based `rank`. Return the ranked list. The watchlist mixes timeframes, so this sort is only sound if `oos_sharpe_mean` is comparable **across timeframes** as well as across strategies: that is guaranteed by the per-timeframe annualisation in `backtest/metrics.py` (WS0-T03 — `√periods_per_year`, D 252 / H4 1512 / H1 6048), not by INV-11. Rows produced before WS0-T03 were annualised √252 for every timeframe and are not comparable; re-run `fathom backtest` before trusting a ranking.
