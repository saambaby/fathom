...
**Enforcement (demo-only measurement / operator-telemetry writes — added Phase 9, extended Phase 8 journal):** Until INV-07 clears, `cli.py` (`fathom execute`) and `signals/analyze.py` (`fathom analyze`) may skip **operator-boundary telemetry** writes when `settings.env == "live"`: append-only `eval.veto_ledger` hooks **and** `operator_journal` UPSERTs (`ai/journal.py::record_journal_entry`). That skip is not a fork of execution/risk/monitoring mechanics: the same `pretrade_check` / `news_risk_check` / sizing / submit code still runs. `eval/`, `ai/journal.py`, and `data/store.py` must not read `settings.env`; the skip lives only at those orchestration call sites. Live verdict/journal recording is out of scope until INV-07.
...
**Rule:** measurement/audit tables (`analysis_log`, `veto_ledger`, the phase-10 trial ledger; any future table whose purpose is recording what happened) expose no UPDATE or DELETE path in the store API — plain `INSERT` only, never `INSERT OR REPLACE`. A wrong row is superseded by a new row from a new run; it is never corrected in place. (Outcome/lifecycle tables — `veto_ledger_outcomes` idempotent refresh upsert, `operator_journal` UPSERT on `client_order_id` — pin their own write semantics in their spec.)
...
INV-02 governs LLM outputs that **feed an automated decision** (vetoes, gates, anything that changes what the system does). Outputs that are **advisory/display-only** (narration, market brief, regime tags, session verdict, companion commentary) take the opposite fallback: a deterministic, non-empty display fallback ("analysis unavailable" / `unavailable` values) — **never** a skip/veto default. Applying INV-02's fail-closed default to an advisory surface silently shrinks output (the failure mode `ai/narration.py`'s CRITICAL docstring warns about); applying the advisory fallback to a veto surface fails open. Every LLM call site must be classified as one or the other in its spec.
