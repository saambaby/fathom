...
| `signals` | `signals/ranker.py`, `signals/portfolio.py`, `signals/correlation.py`, `signals/scan.py`, `signals/analyze.py`, `signals/pine.py` | ranker, portfolio caps; `correlation.py` shared Pearson (Phase 3); `scan.py` = order-free `run_scan` (Phase 4); `analyze.py` / `pine.py` = on-demand analyze + Pine v6 (phase-07). Chart PNG retired in phase-07. | shipped (Phase 2); scan P4; analyze/pine P7 |
...
| `ai` | `ai/llm_client.py`, `news_risk.py`, `narration.py`, `pretrade_check.py`, `brief.py`, `prompts/` | LLM adapter (INV-20) + response models/validators (INV-02); `pretrade_check.py` = in-process pre-trade veto (Phase 3). `jobs/` retired in phase-07. | shipped (Phase 2); `pretrade_check.py` new in Phase 3; package renamed phase-07 |
...
| `cli` | `cli.py` | `fathom backtest` (P1), `scan\|watchlist\|pine\|analyze` (P2/P7), `execute\|positions\|reconcile` (P3), `preflight` (P5) | new in Phase 1 |
...
| `news-risk-assessment` + `watchlist-narration` | both under `ai/` but **different files** (`news_risk.py`+`prompts/news_risk.md` vs `narration.py`+`prompts/narration.md`) | parallel-safe (distinct files); `prompts/` dir is shared but the two files within it don't collide. No `anthropic` dep added (D-P2-3). |
| `hermes-job-definitions` | `jobs/daily.md` (deleted in phase-07 teardown) | **retired (phase-07)** — capstone superseded; acceptance transferred to the pine/analyze walk. |

**Safe-parallel set for Phase 2 (after `signal-ranker` locks the `Candidate` shape):** `{portfolio-limits (portfolio.py)}`, `{chart-generation (charts.py)}`, `{news-risk-assessment (ai/news_risk.py)}`, `{watchlist-narration (ai/narration.py)}` run concurrently — distinct files. `cli-commands (cli.py)` is the join (after ranker+portfolio+charts); `hermes-job-definitions` was the capstone (now retired; acceptance transferred to pine/analyze). `matplotlib` dep via coordinator (removed in phase-07 teardown).
...
| `pretrade-check` | `ai/pretrade_check.py` + `prompts/pretrade.md` | distinct files → parallel-safe with the risk/execution tasks once `Candidate` (shipped) is the only input. |
